...
def perturb_reorder_terms(expr):
    results = []
    parts = []
    depth = 0
    current = []
    ...
    if len(parts) >= 2 and all(p.strip() for p in parts):
        ...
        if new_expr.count('{') == new_expr.count('}'):
            results.append((new_expr, Label.EQUIVALENT, 'adversarial', 'term_reorder_additive'))
    return results

def perturb_reorder_multiplicative(expr):
    results = []
...
def perturb_variable_rename(expr):
    results = []
    for old_var, new_var in [('n', 'm'), ('k', 'j'), ('i', 'l'), ('t', 's')]:
        if f'_{{{old_var}=' in expr or f'_{old_var}' in expr:
            renamed = re.sub(r'(?<![a-zA-Z])' + old_var + r'(?![a-zA-Z])', new_var, expr)
            if renamed != expr:
                results.append((renamed, Label.EQUIVALENT, 'adversarial', f'var_rename_{old_var}_to_{new_var}'))
                break
    return results

def perturb_notation_variant(expr):
    results = []
    if r'\frac{' in expr:
        i = expr.find(r'\frac{')
        if i >= 0:
...
def perturb_latex_formatting(expr):
    results = []
    ...
    results.append((f'The answer is {expr}', Label.EQUIVALENT, 'medium', 'text_wrapper'))
    return results

def perturb_wrong_coefficient(expr):
    results = []
    ...
    if numbers:
        ...
        results.append((new, Label.NOT_EQUIVALENT, 'medium', f'wrong_coeff_{old_num}_to_{new_num}'))
    return results

def perturb_wrong_sign(expr):
    results = []
    if '+' in expr:
        new = expr.replace('+', '-', 1)
...
def perturb_wrong_function(expr):
    results = []
    replacements = [
        (r'\sin', r'\cos'), (r'\cos', r'\sin'),
        (r'\ln', r'\log_{10}'), (r'\sqrt', r'\cbrt'), (r'e^{', r'2^{'),
    ]
    for old, new_func in replacements:
        if old in expr:
            new = expr.replace(old, new_func, 1)
            results.append((new, Label.NOT_EQUIVALENT, 'medium', f'wrong_func_{old}_to_{new_func}'))
            break
    return results

def perturb_wrong_exponent(expr):
    results = []
    ...
    if m:
        ...
        if m.group(0).startswith('^{'):
            ...
            new = expr[:m.start()] + f'^{new_exp}' + expr[m.end():]
        results.append((new, Label.NOT_EQUIVALENT, 'medium', f'wrong_exp_{old}_to_{new_exp}'))
    return results

def perturb_missing_term(expr):
    results = []
...
def perturb_extra_constant(expr):
    results = []
...
def perturb_algebraic_equivalent(expr):
    results = []
    m = re.match(r'\\frac\{(.+)\}\{(.+)\}$', expr.strip())
    if m:
        num, den = m.group(1), m.group(2)
        results.append((f'\\frac{{{num} \\cdot 1}}{{{den}}}', Label.EQUIVALENT, 'easy', 'multiply_by_one'))
    return results

def build_comparable_benchmark():
    instances = []
    id_counter = 0
    equiv_generators = [
        perturb_reorder_terms, perturb_reorder_multiplicative, perturb_variable_rename,
        perturb_notation_variant, perturb_latex_formatting, perturb_algebraic_equivalent,
    ]
    non_equiv_generators = [
        perturb_wrong_coefficient, perturb_wrong_sign, perturb_wrong_function,
        perturb_wrong_exponent, perturb_missing_term, perturb_extra_constant,
    ]
    for answer, domain in REAL_ANSWERS:
        for gen in equiv_generators:
            for pred, label, difficulty, ptype in gen(answer):
                id_counter += 1
                instances.append(BenchInstance(id_counter, answer, pred, label, ptype, difficulty, domain, f"{ptype} on {domain} expr"))
    for answer, domain in REAL_ANSWERS:
    ...
    if len(equiv) > target_equiv:
        ...
        equiv = (adv_equiv + other_equiv)[:target_equiv]
    if len(non_equiv) > target_non_equiv:
        ...
        non_equiv = (adv_non + other_non)[:target_non_equiv]
